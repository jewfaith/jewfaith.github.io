...
export function getAutomaticFestivals() {
    ...
    const RABBINIC_DATES = {
        ...
        'Chag Chanukah': '25-2 Tevet',
        'Chag Hanukkah': '25-2 Tevet',
        'Chanukah': '25-2 Tevet',
        'Hanukkah': '25-2 Tevet',
        'Tzom Tevet': '10 Tevet',
...
export function renderFestivalCard(f) {
    ...
    const isSpecial = f.isBiblical || f.key === 'Yom Shabbat' || (f.title && f.title.toLowerCase().includes('shabbat'));
    return `
        <div class="settings-card event-card glass-panel info-trigger ${isSpecial ? 'special-card' : ''}" 
             tabindex="0" 
             role="button" 
...
export function updateMonthFestivalsList(monthName) {
    ...
    if (uniqueMatches.length === 0) {
        monthContainer.innerHTML = `
            <div class="no-festivals-card glass-panel">
                <i class="fa-solid fa-calendar-check"></i>
                <div>
                    <div class="no-festivals-title">Nenhuma comemoração anual fixa</div>
...
export function applyFestivalFilter() {
    ...
    if (titleEl) titleEl.textContent = 'Festas';
    if (badgeEl) badgeEl.textContent = `${biblicalList.length} Festas`;

    if (rabbinicBadge) rabbinicBadge.textContent = `${rabbinicList.length} Históricas`;
