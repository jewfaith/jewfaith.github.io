    ...
    bodyEl.innerHTML = `
        <div class="levels-container levels-flex-col">
            ...
            <div class="modal-action-box">
                <button class="converter-action-btn w-100" id="activate-premium-from-modal-btn">
 ...
 */
import { getFestivalDescription } from '../domain/constants.js';
import { createDescriptionCardHTML } from './views/dashboardView.js';
...
export function initModals(updateDashboardCallback) {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (infoTrigger) {
            ...
            // Se for clicado dentro de #zmanim-modal ou #day-details-modal, preserva retorno ao modal pai
            const parentModal = infoTrigger.closest('#zmanim-modal, #day-details-modal');
...
export function reopenModals() {
    ...
    try {
        const openOverlay = Array.from(document.querySelectorAll('.modal-overlay')).find(
            m => m.classList.contains('is-open') && !m.classList.contains('is-hidden')
        );
