...
let zmanimRefreshTimer = null;

export function initZmanimModal() {
    if (zmanimRefreshTimer) clearInterval(zmanimRefreshTimer);
    zmanimRefreshTimer = setInterval(() => {
        try {
            renderZmanimTable();
        } catch (e) {}
    }, 60 * 1000);
...
export function renderZmanimTable() {
    ...
    if (zmanimModal && zmanimModal.classList.contains('is-open') && !zmanimModal.classList.contains('is-hidden')) {
        const titleEl = document.getElementById('zmanim-modal-title');
        if (titleEl && (!titleEl.textContent || titleEl.textContent === 'Zmanim')) {
            titleEl.textContent = document.getElementById('solar-hero-city-title')?.textContent?.trim() || "Sha'ah Zmanit";
        }
        const bodyEl = document.getElementById('zmanim-modal-body');
        if (bodyEl) {
            bodyEl.innerHTML = generateZmanimTableHTML();
...
export function generateZmanimTableHTML() {
    ...
    let isYomTovCandleDay = false;
    if (state.unifiedEvents && state.unifiedEvents.length) {
        isYomTovCandleDay = state.unifiedEvents.some(e => {
    ...
    const rawDailyItems = [
        { label: 'Chatzot Layla', desc: 'Meia-noite haláchica', key: 'chatzotNight', val: z.chatzotNight, icon: ICONS.moon, category: 'diario' },
        { label: 'Alot Shachar', desc: 'Alvorecer matinal', key: 'alotHaShachar', val: z.alotHaShachar, icon: ICONS.cloudSun, category: 'diario' },
        { label: 'Alot HaTanya', desc: 'Alvorecer Baal HaTanya', key: 'alosBaalHatanya', val: z.alosBaalHatanya, icon: ICONS.cloudSun, category: 'diario' },
        { label: 'Tempo Misheyakir', desc: 'Talit e Tefilin', key: 'misheyakir', val: z.misheyakir, icon: ICONS.handsPraying, category: 'diario' },
        ...
        { label: 'Netz Chamah', desc: 'Nascer do sol', key: 'sunrise', val: z.sunrise, icon: ICONS.sun, highlight: true, category: 'diario' },
        { label: 'Shemá MGA', desc: 'Limite Magen Avraham', key: 'sofZmanShmaMGA', val: z.sofZmanShmaMGA, icon: ICONS.clock, category: 'diario' },
        { label: 'Shemá HaTanya', desc: 'Limite Baal HaTanya', key: 'sofZmanShmaBaalHatanya', val: z.sofZmanShmaBaalHatanya, icon: ICONS.clock, category: 'diario' },
        { label: 'Shemá GRA', desc: 'Limite Vilna Gaon', key: 'sofZmanShma', val: z.sofZmanShma, icon: ICONS.clock, highlight: true, category: 'diario' },
        { label: 'Tefilah MGA', desc: 'Oração Magen Avraham', key: 'sofZmanTfillaMGA', val: z.sofZmanTfillaMGA, icon: ICONS.hourglass, category: 'diario' },
        { label: 'Tefilah HaTanya', desc: 'Oração Baal HaTanya', key: 'sofZmanTfilaBaalHatanya', val: z.sofZmanTfilaBaalHatanya, icon: ICONS.hourglass, category: 'diario' },
        { label: 'Sof Tefilah', desc: 'Oração Vilna Gaon', key: 'sofZmanTfilla', val: z.sofZmanTfilla, icon: ICONS.hourglass, highlight: true, category: 'diario' },
        { label: 'Chatzot Yom', desc: 'Meio-dia solar', key: 'chatzot', val: z.chatzot, icon: ICONS.compass, highlight: true, category: 'diario' },
        { label: 'Mincha Gedolah', desc: 'Início de Minchá', key: 'minchaGedola', val: z.minchaGedola, icon: ICONS.bell, category: 'diario' },
        { label: 'Gedolah HaTanya', desc: 'Minchá Baal HaTanya', key: 'minchaGedolaBaalHatanya', val: z.minchaGedolaBaalHatanya, icon: ICONS.bell, category: 'diario' },
        { label: 'Mincha Ketanah', desc: 'Minchá preferencial', key: 'minchaKetana', val: z.minchaKetana, icon: ICONS.cloudSun, category: 'diario' },
        { label: 'Ketanah HaTanya', desc: 'Ketanah Baal HaTanya', key: 'minchaKetanaBaalHatanya', val: z.minchaKetanaBaalHatanya, icon: ICONS.cloudSun, category: 'diario' },
        { label: 'Plag Mincha', desc: 'Plag HaMinchá', key: 'plagHaMincha', val: z.plagHaMincha, icon: ICONS.cloudMoon, category: 'diario' },
        { label: 'Plag HaTanya', desc: 'Plag Baal HaTanya', key: 'plagHaminchaBaalHatanya', val: z.plagHaminchaBaalHatanya, icon: ICONS.cloudMoon, category: 'diario' },
        { label: 'Shkiah Solar', desc: 'Pôr do sol', key: 'sunset', val: z.sunset, icon: ICONS.cloudMoon, highlight: true, category: 'diario' },
        { label: 'Bein Hashmashot', desc: 'Intervalo crepuscular', key: 'beinHaShmashos', val: z.beinHaShmashos, icon: ICONS.cloudMoon, category: 'diario' },
        { label: 'Tzeit HaTanya', desc: 'Noite Baal HaTanya', key: 'tzaisBaalHatanya', val: z.tzaisBaalHatanya, icon: ICONS.star, category: 'diario' },
        { label: 'Tzeit Kochavim', desc: 'Saída das estrelas', key: 'tzeit7083deg', val: z.tzeit7083deg, icon: ICONS.star, category: 'diario' },
        { label: 'Tzeit 8.5°', desc: 'Fim do Shabat', key: 'tzeit85deg', val: z.tzeit85deg, icon: ICONS.star, highlight: !isSaturday, category: 'diario' },
        { label: 'Tzeit 42', desc: 'Noite 42 minutos', key: 'tzeit42min', val: z.tzeit42min, icon: ICONS.moon, category: 'diario' },
        { label: 'Tzeit 50', desc: 'Noite 50 minutos', key: 'tzeit50min', val: z.tzeit50min, icon: ICONS.moon, category: 'diario' },
        { label: 'Rabbeinu Tam', desc: 'Noite Rabbeinu Tam', key: 'tzeit72min', val: z.tzeit72min, icon: ICONS.moon, category: 'diario' }
    ];
    ...
    function buildSpecialPrayersForDay(dayOffset) {
        ...
        const targetNowMs = nowMs + offsetMs;
        const dayLocParts = getLocationDateParts(targetNowMs, activeTz);
        const dow = dayLocParts.dayOfWeek;
        const dIsFriday = dow === 5;
        const dIsSaturday = dow === 6;
        const dIsMonday = dow === 1;
        const dIsThursday = dow === 4;

        let dHdate = (dayOffset === 0 && state.currentHdate?.hd) ? state.currentHdate : null;
        if (!dHdate || !dHdate.hd) {
            try {
                const [yStr, mStr, dStr] = (dayLocParts.dateStr || '').split('-');
                dHdate = getHebrewDateFromGregorian(Number(yStr), Number(mStr), Number(dStr)) || {};
            } catch (e) {
                dHdate = hdate || {};
            }
        }

        const dHDay = dHdate.hd ? Number(dHdate.hd) : null;
        const dRawHMonth = dHdate.hm || '';
        const dCanonicalMonth = HEBREW_MONTHS_PT[dRawHMonth] || dRawHMonth;

        const dIsAviv = dCanonicalMonth === 'Aviv' || dRawHMonth === 'Nisan' || dRawHMonth === 'Aviv';
        const dIsZiv = dCanonicalMonth === 'Ziv' || dRawHMonth === 'Iyyar' || dRawHMonth === 'Ziv';
        const dIsSivan = dCanonicalMonth === 'Sivan' || dRawHMonth === 'Sivan';
        const dIsTamuz = dCanonicalMonth === 'Tamuz' || dCanonicalMonth === 'Tammuz' || dRawHMonth === 'Tamuz' || dRawHMonth === 'Tammuz';
        const dIsAv = dCanonicalMonth === 'Av' || dRawHMonth === 'Av';
        const dIsElul = dCanonicalMonth === 'Elul' || dRawHMonth === 'Elul';
        const dIsEtanim = dCanonicalMonth === 'Etanim' || dRawHMonth === 'Tishrei' || dRawHMonth === 'Etanim';
        const dIsKislev = dCanonicalMonth === 'Kislev' || dRawHMonth === 'Kislev';
        const dIsTevet = dCanonicalMonth === 'Tevet' || dRawHMonth === 'Tevet';
        const dIsAdar = dCanonicalMonth === 'Adar' || dCanonicalMonth === 'Adar I' || dCanonicalMonth === 'Adar II' || dRawHMonth.includes('Adar');

        const dDateStr = dayLocParts.dateStr;
        const dEvents = (state.unifiedEvents || []).filter(e => {
            if (!e) return false;
            const evDate = e.raw?.date ? e.raw.date.split('T')[0] : (e.time ? new Date(e.time).toISOString().split('T')[0] : '');
            return evDate === dDateStr;
        });

        const dIsRoshChodesh = (dHDay === 1 || dHDay === 30) || dEvents.some(e => e.category === 'roshchodesh' || e.name?.toLowerCase().includes('rosh chodesh'));
        const dIsYomKippur = (dIsEtanim && dHDay === 10) || dEvents.some(e => e.category === 'yomkippur' || e.name?.toLowerCase().includes('kippur'));
        const dIsYomTeruah = (dIsEtanim && (dHDay === 1 || dHDay === 2)) || dEvents.some(e => e.category === 'yomteruah' || e.name?.toLowerCase().includes('teruah') || e.name?.toLowerCase().includes('rosh hashana'));
        const dIsPesach = (dIsAviv && dHDay !== null && dHDay >= 15 && dHDay <= 21) || dEvents.some(e => e.category === 'pesach' || e.category === 'matzot');
        const dIsShavuot = (dIsSivan && dHDay === 6) || dEvents.some(e => e.category === 'shavuot');
        const dIsSukkot = (dIsEtanim && dHDay !== null && dHDay >= 15 && dHDay <= 21) || dEvents.some(e => e.category === 'sukkot');
        const dIsSheminiAtzeret = (dIsEtanim && dHDay === 22) || dEvents.some(e => e.category === 'sheminiatzeret');
        const dIsChanukah = (dIsKislev && dHDay !== null && dHDay >= 25) || (dIsTevet && dHDay !== null && dHDay <= 2) || dEvents.some(e => e.category === 'chanukah');
        const dIsPurim = (dIsAdar && dHDay === 14) || dEvents.some(e => e.category === 'purim' || e.name?.toLowerCase().includes('purim'));
        const dIsOmer = (dIsAviv && dHDay !== null && dHDay >= 16) || dIsZiv || (dIsSivan && dHDay !== null && dHDay <= 5);
        const dIsCholHaMoed = (dIsAviv && dHDay !== null && dHDay >= 16 && dHDay <= 20) ||
                              (dIsEtanim && dHDay !== null && dHDay >= 16 && dHDay <= 21) ||
                              dEvents.some(e => e.name?.toLowerCase().includes('chol hamoed'));
        const dIsYomTov = (dIsAviv && (dHDay === 15 || dHDay === 21)) ||
                          (dIsSivan && dHDay === 6) ||
                          (dIsEtanim && (dHDay === 1 || dHDay === 10 || dHDay === 15 || dHDay === 22)) ||
                          dEvents.some(e => e.raw?.yomtov === true || (e.isBiblical && ['yomteruah', 'yomkippur', 'sheminiatzeret', 'shavuot', 'matzot', 'sukkot'].includes(e.category)));
        const dIsFastDay = (dIsEtanim && (dHDay === 3 || dHDay === 10)) ||
                           (dIsTevet && dHDay === 10) ||
                           (dIsAdar && dHDay === 13) ||
                           (dIsTamuz && dHDay === 17) ||
                           (dIsAv && dHDay === 9) ||
                           dEvents.some(e => e.category === 'fast' || e.rawCategory === 'fast' || e.name?.toLowerCase().includes('fast') || e.name?.toLowerCase().includes('tzom') || e.name?.toLowerCase().includes("ta'anit"));
        const dIsYomTovEve = (dIsAviv && (dHDay === 14 || dHDay === 20)) ||
                             (dIsSivan && dHDay === 5) ||
                             (dIsElul && dHDay === 29) ||
                             (dIsEtanim && (dHDay === 9 || dHDay === 14 || dHDay === 21));
        const dIsKippurEve = (dIsEtanim && dHDay === 9);
        const dShowCandles = dIsFriday || dIsYomTovEve;

        const dSunriseMs = sunriseMs + offsetMs;
        const dSunsetMs = sunsetMs + offsetMs;
        const dDawnMs = dawnMs + offsetMs;
        const dDuskMs = duskMs + offsetMs;
        const dCandleMs = (toMs(candleTimeVal) || (sunsetMs - 18 * 60 * 1000)) + offsetMs;
        const dHavdalahMs = (toMs(havdalahTimeVal) || (sunsetMs + 42 * 60 * 1000)) + offsetMs;

        const items = [];

        // 1. Seder Selichot (Madrugada em Elul, 10 Dias de Retorno ou Dias de Jejum, exceto Shabat)
        if (!dIsSaturday && (dIsElul || (dIsEtanim && dHDay !== null && dHDay >= 1 && dHDay <= 10) || dIsFastDay)) {
            items.push({
                label: 'Seder Selichot',
                desc: 'Súplicas da madrugada',
                key: 'selichot',
                val: dDawnMs - 60 * 60 * 1000,
                eventEndMs: dSunriseMs,
                icon: ICONS.moon,
                isSpecialDay: true
            });
        }

        // 2. Kriat HaTorah (Shabat, Segunda, Quinta, Rosh Chodesh, Yom Tov, Chol HaMoed, Chanukah, Purim ou Jejum)
        if (dIsSaturday || dIsMonday || dIsThursday || dIsRoshChodesh || dIsYomTov || dIsCholHaMoed || dIsChanukah || dIsPurim || dIsFastDay) {
            items.push({
                label: 'Kriat HaTorah',
        ...
        if (dIsRoshChodesh || dIsPesach || dIsShavuot || dIsSukkot || dIsSheminiAtzeret || dIsChanukah) {
            items.push({
        ...
        // 4. Avinu Malkeinu (Dez Dias de Retorno exceto Shabat, ou Dias de Jejum)
        if ((dIsEtanim && dHDay !== null && dHDay >= 1 && dHDay <= 10 && !dIsSaturday) || (dIsFastDay && !dIsSaturday)) {
            items.push({
        ...
        if (dIsSaturday || dIsRoshChodesh || dIsYomTov || dIsCholHaMoed || dIsFastDay) {
            items.push({
        ...
        if (dIsSaturday || dIsRoshChodesh || dIsYomTov || dIsCholHaMoed || dIsYomKippur) {
            items.push({
                label: 'Tefilat Musaf',
        ...
        // 7. Tekiat Shofar (Yom Teruah / Rosh Hashaná exceto Shabat)
        if (dIsYomTeruah && !dIsSaturday) {
            items.push({
                label: 'Tekiat Shofar',
        ...
        // 8. Tefilat Geshem (Exclusivamente em Shemini Atzeret)
        if (dIsSheminiAtzeret) {
            items.push({
        ...
        // 9. Tefilat Tal (Exclusivamente no 1º dia de Pessach / 15 de Aviv)
        if (dIsAviv && dHDay === 15) {
            items.push({
                label: 'Tefilat Tal',
        ...
        // 10. Mikra Megilah (Exclusivamente em Purim / 14 de Adar)
        if (dIsPurim) {
            items.push({
                label: 'Mikra Megilah',
        ...
        // 11. Minchá Ta'anit (Exclusivamente em Dias de Jejum)
        if (dIsFastDay) {
            items.push({
                label: 'Minchá Ta\'anit',
                desc: 'Minchá de jejum',
                key: 'minchaTaanit',
                val: dSunsetMs - Math.round(2.5 * shaahZmanitMs),
                eventEndMs: dDuskMs,
                icon: ICONS.bell,
                isSpecialDay: true
            });
        ...
        // 12. Tashlich (1º dia de Yom Teruah à tarde, ou 2º dia se o 1º cair em Shabat)
        if (dIsEtanim && ((dHDay === 1 && !dIsSaturday) || (dHDay === 2 && dow === 0))) {
            items.push({
                label: 'Tashlich',
                desc: 'Prece junto às águas',
                key: 'tashlich',
                val: dSunsetMs - Math.round(1.5 * shaahZmanitMs),
                eventEndMs: dSunsetMs,
                icon: ICONS.compass,
                isSpecialDay: true
            });
        ...
        // 13. Kol Nidrei (Véspera de Yom Kippur / 9 de Etanim ao entardecer)
        if (dIsKippurEve) {
            items.push({
                label: 'Kol Nidrei',
                desc: 'Abertura de Yom Kippur',
                key: 'kolNidrei',
                val: dCandleMs,
                eventEndMs: dDuskMs + 90 * 60 * 1000,
                icon: ICONS.star,
                isSpecialDay: true,
                highlight: true
            });
        ...
        // 14. Hadlakat Nerot (Sexta-feira ou Véspera de Yom Tov)
        if (dShowCandles) {
            items.push({
                label: 'Hadlakat Nerot',
                desc: dIsFriday ? 'Velas de Shabat' : 'Velas Festivas',
                key: 'candleLighting',
                val: dCandleMs,
                eventEndMs: dSunsetMs,
                icon: ICONS.candles,
        ...
        // 15. Kabbalat Shabbat (Exclusivamente na Sexta-feira ao entardecer)
        if (dIsFriday) {
            items.push({
                label: 'Kabbalat Shabbat',
                desc: 'Acolhimento do Shabat',
                key: 'kabbalatShabbat',
                val: dCandleMs,
                eventEndMs: dDuskMs,
                icon: ICONS.candles,
        ...
        // 16. Tefilat Neilá (Exclusivamente no Yom Kippur antes do pôr do sol)
        if (dIsYomKippur) {
            items.push({
                label: 'Tefilat Neilá',
                desc: 'Encerramento celeste',
                key: 'neilah',
                val: dSunsetMs - 40 * 60 * 1000,
                eventEndMs: dDuskMs,
                icon: ICONS.star,
                isSpecialDay: true,
                highlight: true
        ...
        // 17. Havdalá Shabat (No Sábado à noite ou encerramento de Yom Tov)
        if (dIsSaturday || (dIsYomTov && !dIsFriday)) {
            items.push({
                label: 'Havdalá Shabat',
                desc: dIsSaturday ? 'Conclusão do Shabat' : 'Conclusão festiva',
                key: 'havdalah',
                val: dHavdalahMs,
                eventEndMs: dHavdalahMs + 30 * 60 * 1000,
                icon: ICONS.star,
                isSpecialDay: true,
                highlight: true
            });
        ...
        // 18. Hadlakat Chanukah (Noites de Chanukah)
        if (dIsChanukah && !dIsFriday) {
            items.push({
                label: 'Hadlakat Chanukah',
                desc: 'Luzes de Chanukah',
                key: 'chanukahLights',
                val: dSunsetMs + 15 * 60 * 1000,
                eventEndMs: dDuskMs + 60 * 60 * 1000,
                icon: ICONS.candles,
                isSpecialDay: true,
                highlight: true
            });
        ...
        // 19. Sefirat HaOmer (49 noites da Contagem do Omer)
        if (dIsOmer) {
            items.push({
                label: 'Sefirat HaOmer',
                desc: 'Contagem do Omer',
                key: 'sefiratHaOmer',
                val: dDuskMs + 10 * 60 * 1000,
                eventEndMs: dDuskMs + 120 * 60 * 1000,
                icon: ICONS.hourglass,
                isSpecialDay: true
            });
        }

        // 20. Kiddush Levana (Do 3º ao 15º dia do mês hebraico)
        if (dHDay !== null && dHDay >= 3 && dHDay <= 15 && !dIsFriday) {
            items.push({
                label: 'Kiddush Levana',
                desc: 'Santificação lunar',
                key: 'kiddushLevana',
                val: dDuskMs + 30 * 60 * 1000,
                eventEndMs: dDuskMs + 90 * 60 * 1000,
                icon: ICONS.moon,
                isSpecialDay: true
            });
    ...
    const specialPrayersItems = Array.from(specialByKey.values());
    ...
    const ZMANIM_KNOWLEDGE = {
        ...
        'Tekiat Shofar': {
            paragraphs: [
                'O mandamento bíblico central de Yom Teruah, Rosh Hashaná, prescrito na Torá em Vayikrá 23:24 e Bamidbar 29:1, cumprido pela audição atenta dos toques sagrados do Shofar.',
                'Os sons de Tekiá, Shevarim e Teruá despertam o coração ao retorno espiritual, proclamando a soberania do Criador e recordando a aliança do Monte Sinai.',
                'Recitam-se duas bênçãos solenes antes do primeiro toque, permanecendo toda a congregação em silêncio reverente até a conclusão da ordem de cem toques.'
            ],
            prayer: {
                hebrew: 'בָּרוּךְ אַתָּה יְהֹוָה אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם, אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֺתָיו וְצִוָּנוּ לִשְׁמֹעַ קוֹל שׁוֹפָר׃',
                translit: 'Baruch Atah Adonai Eloheinu Melech HaOlam, asher kideshanu bemitzvotav vetzivanu lishmoa kol shofar.',
                translation: 'Bendito és Tu, Eterno nosso Deus, Rei do Universo, que nos santificaste com os Teus mandamentos e nos ordenaste ouvir a voz do Shofar.'
            }
        },
        'Tashlich': {
            paragraphs: [
                'Cerimónia penitencial realizada na tarde do primeiro dia de Yom Teruah junto a águas correntes, rios ou ao mar, inspirada nas palavras do profeta Miqueias.',
                'Simboliza a purificação interior e o lançamento de todas as faltas às profundezas das águas, renovando o compromisso de caminhar em retidão perante o Eterno.',
                'Se o primeiro dia coincidir com o sagrado Shabat, a prece de Tashlich é transferida para a tarde do segundo dia.'
            ],
            prayer: {
                hebrew: 'מִי אֵל כָּמוֹךָ נֹשֵׂא עָוֺן וְעֹבֵר עַל־פֶּשַׁע לִשְׁאֵרִית נַחֲלָתוֹ, לֹא־הֶחֱזִיק לָעַד אַפּוֹ כִּי־חָפֵץ חֶסֶד הוּא׃ יָשׁוּב יְרַחֲמֵנוּ, יִכְבֹּשׁ עֲוֺנֹתֵינוּ, וְתַשְׁלִיךְ בִּמְצֻלוֹת יָם כָּל־חַטֹּאותָם׃',
                translit: 'Mi El kamocha noseh avon ve\'over al pesha lish\'erit nachalato, lo hechezik la\'ad apo ki chafetz chessed Hu. Yashuv yerachameinu, yichbosh avonoteinu, vetashlich bimtzulot yam kol chatotam.',
                translation: 'Quem é Deus como Tu, que perdoa a iniquidade e passa por cima da transgressão do remanescente da Sua herança? Ele não retém para sempre a Sua ira, porque Se compraz na benevolência. Ele tornará a compadecer-Se de nós, subjugará as nossas iniquidades e lançará nas profundezas do mar todos os nossos pecados.'
            }
        },
        'Kol Nidrei': {
            paragraphs: [
                'A solene proclamação que inaugura a noite sagrada de Yom Kippur na sinagoga, entoada antes do pôr do sol perante os rolos da Torá erguidos junto ao oficiante.',
                'Anula votos, promessas e compromissos involuntários entre a pessoa e o Criador, preparando a alma para ingressar em pureza no Dia da Expiação.',
                'Conclui-se com a bênção de Shehecheyanu e a entrada solene no jejum e nas preces de Ma\'ariv de Yom Kippur.'
            ],
            prayer: {
                hebrew: 'וְנִסְלַח לְכָל־עֲדַת בְּנֵי יִשְׂרָאֵל וְלַגֵּר הַגָּר בְּתוֹכָם, כִּי לְכָל־הָעָם בִּשְׁגָגָה׃ סְלַח־נָא לַעֲוֺן הָעָם הַזֶּה כְּגֹדֶל חַסְדֶּךָ, וְכַאֲשֶׁר נָשָׂאתָה לָעָם הַזֶּה מִמִּצְרַיִם וְעַד־הֵנָּה׃ וַיֹּאמֶר יְהוָה: סָלַחְתִּי כִּדְבָרֶךָ׃',
                translit: 'Venislach lechol adat benei Yisrael velager hagar betocham, ki lechol ha\'am bishgagah. Selach na la\'avon ha\'am hazeh kegodel chasdecha, vecha\'asher nasatah la\'am hazeh miMitzrayim ve\'ad henah. Vayomer Adonai: Salachti kidvarecha.',
                translation: 'E será perdoada toda a congregação dos filhos de Israel e o estrangeiro que habita no meio deles, pois todo o povo agiu por inadvertência. Perdoa, pois, a iniquidade deste povo segundo a grandeza da Tua benevolência, assim como perdoaste a este povo desde o Egito até aqui. E disse o Eterno: Perdoei conforme a tua palavra.'
            }
        },
        'Hadlakat Chanukah': {
            paragraphs: [
                'Acendimento festivo das luzes da Chanukiá durante as oito noites de Chanukah, proclamando o milagre da dedicação do Templo Sagrado de Jerusalém.',
                'A cada noite acrescenta-se uma nova chama, posicionada para irradiar luz espiritual e gratidão pelas maravilhas operadas pelo Eterno em favor dos Seus filhos.',
                'Após as bênçãos, entoa-se Hanerot Halalu e Maoz Tzur, recordando que as luzes são sagradas e destinadas exclusivamente à contemplação e ao louvor.'
            ],
            prayer: {
                hebrew: 'בָּרוּךְ אַתָּה יְהֹוָה אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם, אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֺתָיו וְצִוָּנוּ לְהַדְלִיק נֵר שֶׁל חֲנֻכָּה׃ בָּרוּךְ אַתָּה יְהֹוָה אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם, שֶׁעָשָׂה נִסִּים לַאֲבוֹתֵינוּ בַּיָּמִים הָהֵם בַּזְּמַן הַזֶּה׃',
                translit: 'Baruch Atah Adonai Eloheinu Melech HaOlam, asher kideshanu bemitzvotav vetzivanu lehadlik ner shel Chanukah. Baruch Atah Adonai Eloheinu Melech HaOlam, she\'asah nissim la\'avoteinu bayamim hahem bazman hazeh.',
                translation: 'Bendito és Tu, Eterno nosso Deus, Rei do Universo, que nos santificaste com os Teus mandamentos e nos ordenaste acender a luz de Chanukah. Bendito és Tu, Eterno nosso Deus, Rei do Universo, que realizaste milagres pelos nossos antepassados naqueles dias, nesta época.'
            }
        },
        'Mikra Megilah': {
            paragraphs: [
                'Leitura pública integral do Rolo de Ester, Meguilat Ester, na noite e na manhã da festa de Purim, celebrando a providência divina que salvou o povo de Israel.',
                'O pergaminho é desdobrado como uma epístola solene antes das três bênçãos iniciais, proclamando a transformação do luto em alegria e das trevas em luz.',
                'Acompanha os demais preceitos de Purim: o envio de porções de alimentos aos amigos, Mishloach Manot, e dádivas generosas aos necessitados, Matanot LaEvyonim.'
            ],
            prayer: {
                hebrew: 'בָּרוּךְ אַתָּה יְהֹוָה אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם, אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֺתָיו וְצִוָּנוּ עַל מִקְרָא מְגִלָּה׃',
                translit: 'Baruch Atah Adonai Eloheinu Melech HaOlam, asher kideshanu bemitzvotav vetzivanu al mikra Megilah.',
                translation: 'Bendito és Tu, Eterno nosso Deus, Rei do Universo, que nos santificaste com os Teus mandamentos e nos ordenaste sobre a leitura da Meguilá.'
            }
        },
        'Sefirat HaOmer': {
            paragraphs: [
                'O mandamento bíblico de contar os quarenta e nove dias e sete semanas completas desde a oferta do Omer em Pessach até a festa da entrega da Torá em Shavuot.',
                'Realizada de pé logo ao anoitecer, após Tzeit HaKochavim, enumerando tanto os dias quanto as semanas que conduzem à preparação espiritual para o Sinai.',
                'Constitui uma jornada ascendente de refinamento interior e expectativa sagrada pela recepção da Palavra Divina.'
            ],
            prayer: {
                hebrew: 'בָּרוּךְ אַתָּה יְהֹוָה אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם, אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֺתָיו וְצִוָּנוּ עַל סְפִירַת הָעֹמֶר׃',
                translit: 'Baruch Atah Adonai Eloheinu Melech HaOlam, asher kideshanu bemitzvotav vetzivanu al sefirat haOmer.',
                translation: 'Bendito és Tu, Eterno nosso Deus, Rei do Universo, que nos santificaste com os Teus mandamentos e nos ordenaste sobre a contagem do Omer.'
            }
        }
    };
    ...
    const createZmanDescriptionHTML = (label, desc, time, isTomorrow) => {
        ...
        return `
            <div class="levels-container zmanim-modal-levels">
                ...
                `).join('')}
                ${prayer ? `
                    <div class="info-modal-card info-card-col">
                        <div class="info-modal-value zmanim-modal-hebrew" dir="rtl">
                            <span class="hebrew-text">${prayer.hebrew}</span>
                        </div>
                        <div class="info-modal-value zmanim-modal-translit">
                            ${prayer.translit}
                        </div>
                        <div class="info-modal-value zmanim-modal-desc">
                            ${prayer.translation}
                        </div>
                    </div>
                ` : ''}
    ...
    return `
        <div class="calendar-modal-content zmanim-modal-container">
