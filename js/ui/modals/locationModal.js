...
export function renderSuggestions(results = [], options = {}) {
    ...
    finalItems.forEach(resItem => {
        ...
        const cleanPrimary = String(primaryText || 'Localidade').replace(/\s*\([^)]*\)/g, '').trim();
        const cleanSecondaryDesc = String(secondaryText || 'Localidade').replace(/\s*\([^)]*\)/g, '').trim() || 'Localidade';

        li.setAttribute('aria-label', cleanPrimary);

        li.innerHTML = `
            <div class="settings-card-left">
                <i class="${iconClass} settings-icon"></i>
                <div class="settings-card-text">
                    <span class="settings-card-title">${escapeHtml(cleanPrimary)}</span>
                    <span class="settings-card-desc">${escapeHtml(cleanSecondaryDesc)}</span>
                </div>
            </div>
        ...
        li.addEventListener('click', async () => {
            ...
            if (resItem.isDeviceGpsAction) {
                ...
                if (descEl) descEl.textContent = 'Sinal GPS';

                const gpsLoc = await getDevicePreciseLocation(6000);
                if (gpsLoc) {
                    locObj = gpsLoc;
                } else {
                    if (descEl) descEl.textContent = 'GPS Indisponível';
                    return;
                }
...
export function openLocationModal(options = {}) {
    ...
    if (readingBody) readingBody.classList.add('is-hidden');
    ...
    setTimeout(() => {
        if (skeletonContainer) skeletonContainer.classList.add('is-hidden');
        if (readingBody) readingBody.classList.remove('is-hidden');
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        searchTimeout = setTimeout(async () => {
            ...
            if (!data) {
                ...
                if (suggestionsList) {
                    suggestionsList.innerHTML = `
                        <li class="settings-card glass-panel location-empty-card">
