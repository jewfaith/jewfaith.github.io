...
import { escapeHtml, ensureTwoWords } from '../../domain/formatters.js';
...
export async function openReadingModal(ref, cardTitle, options = {}) {
    ...
    const resolvedTitle = (cardTitle && cardTitle.trim()) ? cardTitle.trim() : (ref || 'Leitura Sagrada');
    ...
    try {
        ...
        titleEl.textContent = (cardTitle && cardTitle.trim()) ? cardTitle.trim() : (ref || 'Leitura Sagrada');
            ...
            html += `
                <div class="legend-card verse-card">
                    <div class="verse-body">
                        <div class="verse-text"><strong class="verse-num">${displayNum}</strong> ${escapeHtml(v.text)}</div>
                    </div>
                </div>
            `;
        }
