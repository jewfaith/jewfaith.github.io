...
import { escapeHtml, formatHebrewInText, ensureTwoWords } from '../../domain/formatters.js';
...
export async function openSefariaModal(category = null, targetRef = null, options = {}) {
    ...
    const exactCardTitle = (options.cardTitle && options.cardTitle.trim()) ||
        document.getElementById('card-sefaria-title')?.textContent?.trim() ||
        partTitle ||
        'Estudo Sefaria';
    ...
    if (titleEl) {
        titleEl.textContent = exactCardTitle;
    }
    if (subtitleEl) {
    ...
    trackMicroAction('modal_open', { modal: 'sefaria_reading', ref: exactCardTitle });
    ...
    await renderSefariaModalView(targetRef, category, exactCardTitle);
}
...
function renderLoadingState() {
    return `
        <div class="thematic-loader-container sefaria-loader-box">
 ...
 */
async function renderSefariaModalView(targetRef = null, category = null, lockedCardTitle = null) {
    const body = document.getElementById('sefaria-modal-body');
    if (!body) return;
    ...
    try {
        ...
        if (reading && reading.ok) {
            const titleEl = document.getElementById('sefaria-modal-title');
            if (titleEl) {
                const readingTitle = lockedCardTitle ||
                    document.getElementById('card-sefaria-title')?.textContent?.trim() ||
                    formatShortLiteratureTitle(reading.ref || reading.title || '');
                titleEl.textContent = readingTitle || 'Estudo Sefaria';
            }
        }
...
function renderReadingContent(reading) {
    if (!reading || !reading.ok) {
        return `
            <div class="reading-error reading-error-box">
                <i class="fa-solid fa-triangle-exclamation reading-error-icon"></i>
                <span class="reading-error-title reading-error-title-block">
                    ${escapeHtml(reading?.error || 'Erro na Leitura')}
                </span>
                <p class="reading-error-desc">
                    Não foi possível carregar o texto da Sefaria no momento. Tente novamente.
                </p>
                <button type="button" id="sefaria-retry-btn" class="settings-card glass-panel sefaria-retry-button">
    ...
    const paragraphsHtml = (reading.paragraphs || []).map((p, idx) => {
        ...
        return `
            <div class="legend-card verse-card">
                <div class="verse-body">
                    <div class="verse-text"><strong class="verse-num">${displayNum}</strong> ${formatHebrewInText(escapeHtml(cleanText))}</div>
                </div>
            </div>
        `;
    }).join('');
