 ...
 * Carregado de forma assíncrona e não-bloqueante para reduzir o bundle crítico inicial em ~957 KB (otimização LCP e INP).
 */
export const FESTIVAL_DESCRIPTIONS = {};
let _getFestivalDescriptionFn = null;
let _festivalDescriptionsPromise = null;

export function getFestivalDescription(nameOrKey) {
    if (!nameOrKey) return null;
    if (_getFestivalDescriptionFn) {
        return _getFestivalDescriptionFn(nameOrKey);
    }
    const trimmed = String(nameOrKey).trim();
    const mapped = FESTIVAL_NAME_MAPPINGS?.[trimmed] || trimmed;
    return FESTIVAL_DESCRIPTIONS[mapped] || FESTIVAL_DESCRIPTIONS[trimmed] || null;
}

export function loadFestivalDescriptions() {
    if (_festivalDescriptionsPromise) return _festivalDescriptionsPromise;
    _festivalDescriptionsPromise = import('./festivalDescriptions.js')
        .then((mod) => {
            if (mod.FESTIVAL_DESCRIPTIONS) {
                Object.assign(FESTIVAL_DESCRIPTIONS, mod.FESTIVAL_DESCRIPTIONS);
            }
            if (typeof mod.getFestivalDescription === 'function') {
                _getFestivalDescriptionFn = mod.getFestivalDescription;
            }
            return FESTIVAL_DESCRIPTIONS;
        })
        .catch(() => FESTIVAL_DESCRIPTIONS);
    return _festivalDescriptionsPromise;
}
