...
import { updateSolarPosition, updateShaahZmanitCardPosition } from '../ui/solarArc.js';
...
function performHeartbeatCheck() {
    ...
    // Mantém o arco solar, a posição do Sha'ah Zmanit e a tabela modal sincronizados
    try {
        updateSolarPosition();
        updateShaahZmanitCardPosition();
        const zModal = document.getElementById('zmanim-modal');
        if (zModal && zModal.classList.contains('is-open') && !zModal.classList.contains('is-hidden')) {
            renderZmanimTable();
        }
    } catch (e) { }
...
export function isUserReadingOrInteracting() {
    ...
    const openModal = document.querySelector('.modal-overlay.is-open:not(.is-hidden)');
    return !!openModal;
}
...
function evaluateFreshness(triggerSource) {
    ...
    // Mantém sempre o arco solar e Sha'ah Zmanit alinhados instantaneamente
    try {
        updateSolarPosition();
        updateShaahZmanitCardPosition();
    } catch (e) { }
