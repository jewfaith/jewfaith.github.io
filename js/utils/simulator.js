...
function buildMonthEvents(simCategory, dayIndex, now, hYear = 5787) {
    ...
    const createEvt = (name, cat, offsetDays, isBiblical, isYomTov, hDay, hMonth, extra = {}) => {
        const time = now + (offsetDays * dayMs) - 12 * 3600 * 1000;
        return {
            name,
            category: cat,
            time,
    ...
    if (simCategory === 'yomteruah') {
        ...
        // 2 Tishrei: 2º dia (Tradição Rabínica - não é Yom Tov da Torá)
        events.push(createEvt('Rosh Hashana', 'roshhashana', dOffset + 1, false, false, 2, 'Tishrei'));
...
export function simulateFestival(nameOrDisable, opt = 1) {
    ...
    state.simulatedTime = new Date(simGregorian).getTime();
...
export async function simulateDate(isoDateStr) {
    ...
    try {
        ...
        const hebcalUrl = `https://www.hebcal.com/hebcal?v=1&cfg=json&geo=pos&latitude=${loc.lat}&longitude=${loc.lon}&start=${year}-01-01&end=${year + 1}-12-31&maj=on&min=on&mod=on&nx=on&mf=on&ss=on&s=on&i=${loc.isIsrael ? 'on' : 'off'}&c=off&o=on`;
        ...
        state.simulatedTime = new Date(`${dateStr}T12:00:00`).getTime();
